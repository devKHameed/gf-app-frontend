...
import AccountTypeModel from "models/AccountTypes";
import { queryClient } from "../";
import { keys } from "../keyNames";

export default function useUpdate() {
  return useMutation((values) => AccountTypeModel.update(values.slug, values), {
    onMutate: async (value: AccountType) => {
      ...
      // Optimistically update by updating the value
      if (backup)
        queryClient.setQueryData<{ data: AccountType[] }>([keys.accountType], {
          data: [...backup.data.map((h) => (h.id === value.id ? value : h))],
        });
