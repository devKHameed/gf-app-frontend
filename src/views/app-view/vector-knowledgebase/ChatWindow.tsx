...
import { useVectorKnowledgebaseStore } from "store/stores/vecotr-knowledgebase";
...
type MessageBoxProps = {
  message: VectorKnowledgebaseMessage;
};
...
const ChatMessages: React.FC<ChatMessagesProps> = (props) => {
  const messages = useVectorKnowledgebaseStore.useMessages();
...
const ChatWindow: React.FC<ChatWindowProps> = (props) => {
  ...
  const addMessage = useVectorKnowledgebaseStore.useAddMessage();
  const updateMessage = useVectorKnowledgebaseStore.useUpdateMessage();
  const setMessages = useVectorKnowledgebaseStore.useSetMessages();

  const scrollbarRef = React.useRef<Scrollbars>(null);

  useEffect(() => {
    if (slug) {
      VectorKnowledgebaseModel.listMessages(slug).then((res) => {
        setMessages(
          res.data.sort((a, b) =>
  ...
  const handleSubmit = async (value: string) => {
    if (slug) {
      ...
      const { data } = await VectorKnowledgebaseModel.createMessage(
        slug,
        value
      );
