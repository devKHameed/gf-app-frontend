...
const useHotkeyFunc = () => {
  const { removeCanvasItem, createCanvasItem, updateCanvasItem } = useItem();
  ...
  const copyItems = (
    ...
    setClipboard: (value: React.SetStateAction<StageDataItem[]>) => void,
    createStageDataObject: (item: Node<NodeConfig>) => StageDataItem
  ) => {
  ...
  const pasteItems = (clipboard: StageDataItem[]) => {
  ...
  const deleteItems = (
    ...
    removeCanvasItem(selectedItems.map((item) => item.id()));
  };

  const layerUp = (selectedItems: Node<NodeConfig>[]) => {
    selectedItems.forEach((item) => {
      item.moveUp();
      updateCanvasItem(item.id(), (prevData) => ({
        ...item.attrs,
        zIndex: 1,
        updatedAt: Date.now(),
      }));
